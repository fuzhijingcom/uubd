...
$(function(){
    $('body').on('click', '.topic_thank', function(e) {
        var statu = confirm("你确定要向本主题创建者发送谢意？");
        if(!statu){
            return false;
        }else{
        self = $(this);
        ...
        $.ajax({
            ...
            success: function (data) {
              if(data.result == 1) {
                ...
                $('#topic_thank').html('<span class="thanks f11 gray" style="text-shadow: 0px 1px 0px #fff;">感谢已发送</span>');
                }
            });
    }
    });
...
$(function(){
    $('body').on('click', '.comment_thank', function(e) {
        var statu = confirm("确认花费 20 个铜币向这条回复发送感谢？");
        if(!statu){
            return false;
        }else{
            self = $(this);
            ...
            $.ajax({
                ...
                success: function (data) {
                  if(data.result == 1) {
                    ...
                    $('#thank_area_'+params[2]).html('<div class="thanked">感谢已发送</div>');
                }
            });
    }
    });
...
$(function(){
    $('body').on('click', '.favorite', function(e) {
        var title = {
            'favorite':{'user':'取消特别关注','topic':'取消收藏', 'node':'取消收藏','vote_topic':'取消赞','vote_comment':'取消赞'},
            'unfavorite':{'user':'加入特别关注',topic:'加入收藏', 'node':'加入收藏','vote_topic':'赞','vote_comment':'赞'}
        };
        self = $(this);
