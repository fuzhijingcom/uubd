            ...
            if ($user = $model->signup()) {
                if (Yii::$app->getUser()->login($user)) {
                    if ($user->status == User::STATUS_INACTIVE) {
                        return $this->render('opResult', ['title'=>'帐号注册成功', 'status'=>'message', 'msg'=>'感谢您的注册，激活邮件已发送到您的注册邮箱。请去点击激活。']);
                    } else if ($user->status == User::STATUS_ADMIN_VERIFY) {
                        return $this->render('opResult', ['title'=>'帐号注册成功', 'status'=>'message', 'msg'=>'感谢您的注册，请等待管理员确认。']);
                    } else {
                        return $this->goHome();
                    }
                }
